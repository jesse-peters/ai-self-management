...
-- Add hash column for JWT access tokens if table exists and column doesn't
ALTER TABLE IF EXISTS oauth_tokens 
  ADD COLUMN IF NOT EXISTS access_token_hash TEXT;

-- Create index for faster JWT lookups (only if table exists)
DO $$ BEGIN
...
-- Authorization Codes (if table exists)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'authorization_codes') THEN
