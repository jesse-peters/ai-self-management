...
export const tools: Tool[] = [
  ...
  // Work Items
  {
    name: 'pm.work_item.create',
    description: 'Creates a new work item (external ticket reference) for a project',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectId: { type: 'string', description: 'Project ID' },
  ...
  {
    name: 'pm.work_item.list',
    description: 'Lists work items for a project with optional filters',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectId: { type: 'string', description: 'Project ID' },
  ...
  {
    name: 'pm.work_item.get',
    description: 'Gets a work item with tasks summary and gate status',
    inputSchema: {
      type: 'object' as const,
      properties: {
  ...
  {
    name: 'pm.work_item.set_status',
    description: 'Updates work item status (enforces gate rule - required gates must pass before marking done)',
    inputSchema: {
      type: 'object' as const,
      properties: {
        workItemId: { type: 'string', description: 'Work item ID' },
  ...
  {
    name: 'pm.task.create',
    description: 'Creates a new agent task (micro work packet) with dependencies and verification',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectId: { type: 'string', description: 'Project ID' },
  ...
  {
    name: 'pm.task.list',
    description: 'Lists agent tasks for a project with optional filters',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectId: { type: 'string', description: 'Project ID' },
        workItemId: { type: 'string', description: 'Filter by work item ID' },
        status: {
          type: 'string',
  ...
  {
    name: 'pm.task.get',
    description: 'Gets an agent task with evidence, dependencies, and gate runs',
    inputSchema: {
      type: 'object' as const,
      properties: {
        taskId: { type: 'string', description: 'Task ID' },
  ...
  {
    name: 'pm.task.add_dependency',
    description: 'Adds a dependency between two tasks',
    inputSchema: {
      type: 'object' as const,
      properties: {
        taskId: { type: 'string', description: 'Task ID' },
        dependsOnTaskId: { type: 'string', description: 'Task ID that must be completed first' },
        userId: { type: 'string', description: 'User ID (optional if set in env)' },
      },
  ...
  {
    name: 'pm.evidence.list',
    description: 'Lists evidence for a task or work item',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectId: { type: 'string', description: 'Project ID' },
  ...
  // Gates
  {
    name: 'pm.gate.configure',
    description: 'Configures gates (verification checks) for a project',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectId: { type: 'string', description: 'Project ID' },
        gates: {
          ...
          items: {
            type: 'object',
            properties: {
              name: { type: 'string', description: 'Gate name (e.g., "tests", "lint")' },
              is_required: { type: 'boolean', description: 'Whether this gate is required' },
              runner_mode: {
                type: 'string',
                enum: ['manual', 'command'],
                description: 'How to run the gate',
              },
              command: { type: 'string', description: 'Command to run (required if runner_mode is "command")' },
            },
            required: ['name', 'is_required', 'runner_mode'],
          },
