...
import { ValidationError, ProjectFlowError } from '@projectflow/core';
import { withErrorHandler } from '@/lib/api/withErrorHandler';
import { createSuccessResponse, createErrorResponse } from '@/lib/errors/responses';
...
export const GET = withErrorHandler(async (request: NextRequest): Promise<NextResponse> => {
    ...
    if (process.env.NODE_ENV !== 'development') {
        // Return OAuth-style error response for consistency
    ...
    if (!jwtSecret) {
        throw new ProjectFlowError('JWT secret not configured', 'CONFIGURATION_ERROR');
    }
