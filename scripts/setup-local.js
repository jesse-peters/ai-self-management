...
function getSupabaseStatus() {
  try {
    ...
    return {
      running: true,
      apiUrl: status?.API?.URL || "http://localhost:54321",
    ...
    return {
      running: false,
      apiUrl: process.env.NEXT_PUBLIC_SUPABASE_URL || "http://localhost:54321",
...
async function main() {
  ...
  if (
    !checkCommand(
      "pnpm",
      "❌ pnpm is not installed. Please install it first: npm install -g pnpm"
    )
  ) {
    process.exit(1);
  }
  ...
  if (
    !checkCommand(
      "docker",
      "⚠️  Docker is not running. Starting Supabase may fail."
    )
  ) {
  ...
  if (!supabaseStatus.running) {
    ...
    if (!supabaseStatus.running) {
      log(
        "⚠️  Supabase started but status check failed. Using defaults.",
        YELLOW
      );
    }
  } else {
  ...
  if (!runCommand("pnpm --filter @projectflow/db build", process.cwd(), true)) {
    log(
      "⚠️  Failed to build @projectflow/db (may be okay if already built)",
      YELLOW
    );
  }
  if (
    !runCommand("pnpm --filter @projectflow/core build", process.cwd(), true)
  ) {
    log(
      "⚠️  Failed to build @projectflow/core (may be okay if already built)",
      YELLOW
    );
  }
  log("✓ Packages built\n", GREEN);
  ...
  if (!fs.existsSync(envFile)) {
    ...
    if (!supabaseStatus.anonKey || !supabaseStatus.serviceKey) {
      try {
        ...
        const serviceMatch = statusOutput.match(
          /service_role key:\s*([^\s]+)/i
        );
        ...
        log(
          "⚠️  Could not auto-detect Supabase keys. You may need to update .env.local manually.",
          YELLOW
        );
      }
    }
